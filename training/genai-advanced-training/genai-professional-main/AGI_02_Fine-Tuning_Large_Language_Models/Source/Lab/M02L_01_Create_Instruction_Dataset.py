...
# train_df.write.mode("overwrite").saveAsTable(<FILL_IN>)
# eval_df.write.mode("overwrite").saveAsTable(<FILL_IN>)
...
# _ = spark.sql(f"CREATE VOLUME IF NOT EXISTS {CATALOG}.{USER_SCHEMA}.{OUTPUT_VOLUME}")

# # Convert the UC train table to single JSONL file
# write_table_to_volume_as_jsonl(
#     catalog=<FILL_IN>,
#     schema=<FILL_IN>,
#     table_name=f"<FILL_IN>",
#     volume=<FILL_IN>,
#     output_file_name=<FILL_IN>
# )
# '
...
write_table_to_volume_as_jsonl(
    catalog=CATALOG,
    schema=USER_SCHEMA,
    table_name=f"{CATALOG}.{USER_SCHEMA}.{OUTPUT_TRAIN_TABLE}",
