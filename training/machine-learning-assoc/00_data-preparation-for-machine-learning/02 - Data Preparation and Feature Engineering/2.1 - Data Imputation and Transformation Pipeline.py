...
# MAGIC %md
# MAGIC Casting didn't work on `SeniorCitizen` most probably because there was some null values or values which couldn't be encoded correctly, we ca force coerce using a simple filter method (assuming missing values in this column can be encoded as `False`)
...
# MAGIC
# MAGIC * Comparing the count of records before and after by divinding the count of **`telco_casted_full_df`** and **`telco_no_outliers_df`** dataframe removing outliers and then materializing the resulting dataframe as a new table.
...
# MAGIC
# MAGIC To Handle missing values in dataset we need to identify columns with high percentages of missing data and drops those columns. Then, it removes rows with missing values. Numeric columns are imputed with 0, and string columns are imputed with 'N/A'. Overall, the code demonstrates a comprehensive approach to handling missing values in the dataset.
...
# MAGIC %md
# MAGIC **Replace numeric missing with constant/0**
# MAGIC
# MAGIC NOTE: not applicable in this dataset's case
...
# MAGIC
# MAGIC <img src="https://files.training.databricks.com/images/icon_hint_24.png"/>&nbsp;**Hint:** Check out the [**`randomSplit`** documentation](https://spark.apache.org/docs/3.1.3/api/python/reference/api/pyspark.sql.DataFrame.randomSplit.html).
