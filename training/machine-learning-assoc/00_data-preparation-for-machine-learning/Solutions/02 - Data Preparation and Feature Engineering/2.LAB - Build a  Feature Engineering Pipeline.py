...
MentHlth_cutoff = -40
cdc_no_outliers_df = cdc_no_missing_df.filter(col("MentHlth") <= MentHlth_cutoff)  # Use SQL col() function

# Remove listings with BMI > 110
BMI_cutoff = 110
cdc_no_outliers_df = cdc_no_missing_df.filter(col("BMI") <= BMI_cutoff)
