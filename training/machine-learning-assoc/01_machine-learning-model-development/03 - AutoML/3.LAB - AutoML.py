...
# MAGIC
# MAGIC * **Task 2 :** Create a classification experiemnt using the AutoMl UI.
# MAGIC
...
# MAGIC %run ../Includes/Classroom-Setup-03.LAB
...
# MAGIC
# MAGIC   ***Step 2.*** Click on **Create AutoML Experiment** in the top-right corner.
# MAGIC
# MAGIC   ***Step 3.*** Choose a cluster for experiment execution.
# MAGIC
# MAGIC   ***Step 4.*** For the ML problem type, select **`Classification`**.
# MAGIC
# MAGIC   ***Step 5.*** Select the input training dataset as **`catalog > schema > bank_loan`**.
# MAGIC
# MAGIC   ***Step 6.*** Specify **`Personal_Loan`** as the prediction target.
# MAGIC
# MAGIC   ***Step 7.*** Deselect the **`ID`**, **`ZIP_Code`** field as it's not needed as a feature.
# MAGIC
# MAGIC   ***Step 8.*** In the **Advanced Configuration** section, set the **Timeout** to **5 minutes**.
# MAGIC
...
summary = automl.classify(
    dataset = <FILL_IN>,
    target_col = <FILL_IN>,
    exclude_cols=<FILL_IN>,  # Exclude columns as needed
